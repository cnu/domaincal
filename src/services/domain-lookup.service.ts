...
const logger = createLogger("DomainLookupService");
...
export class DomainLookupService {
  ...
  private static validateApiKey(): void {
    if (!this.API_KEY || this.API_KEY.length < 32) {
      logger.error("WHOIS API key is not set or appears invalid", {
        hasKey: !!this.API_KEY,
  ...
  static async getDetailedWhoisInfo(
    ...
    try {
      ...
      logger.info("WHOIS API Configuration:", {
      ...
      logger.info(`Fetching WHOIS data for ${domainName}...`);
      ...
      if (!response) {
        throw new Error(
          `No response received from WHOIS API for domain ${domainName}`
        );
      }
      ...
      if (response.status === 429) {
        // Rate limit hit
        logger.warn(
          `Rate limit hit for domain ${domainName}, backing off...`
        );
      ...
      if (!response.ok) {
        ...
        logger.error(`WHOIS API error for ${domainName}:`, errorDetails);        throw new Error(
          `WHOIS API failed with status ${response.status}: ${errorText}`
        );
      }
      ...
      if (!whoisData || typeof whoisData !== "object") {
        throw new Error(
          `Invalid WHOIS API response for ${domainName}: ${JSON.stringify(
            whoisData
          )}`
        );
      }
      ...
      logger.error(`Error fetching WHOIS data for ${domainName}:`, error);
      throw new Error(
        `Failed to fetch WHOIS data for domain ${domainName}: ${
  ...
  static async updateDomainInfo(
    ...
    try {
      ...
      try {
        ...
        logger.error(`Error fetching domain with ID ${domainId}:`, error);
        return {
          success: false,
      ...
      if (!forceRefresh && domainWithRefresh.lastRefreshedAt) {
        ...
        const hasSuccessfulWhoisResponse =
          domainWithRefresh.response &&
        ...
        if (now < cooldownEnds) {
          ...
          if (hasWhoisResponse) {
            ...
            timeMessage = `${secondsRemaining} second${
